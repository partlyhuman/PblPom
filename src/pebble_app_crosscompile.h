...
// Add support for cross-compilation on platforms that don't have the appropriate file segment
// e.g. Mach-O
#ifndef SIMULATOR_MAC
#define __PBL_SEGMENT_ATTRIBUTE __attribute__ ((section (".pbl_header")))
#else
#define __PBL_SEGMENT_ATTRIBUTE
#endif
...
#define PBL_APP_INFO(_uuid, _name, _company, _version_major, _version_minor, _icon_resource_id, _flags)    \
  const PebbleAppInfo __pbl_app_info __PBL_SEGMENT_ATTRIBUTE = { \
    .header = "PBLAPP", \
